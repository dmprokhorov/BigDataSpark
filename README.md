...
# Инструкция
1. Прописать в консоли команду `docker-compose up -d`
2. Далее надо прописать команду `docker logs bigdata_jp`, там можно будет получить ссылку на ссылку на среду Jupyter (она будет выглядеть примерно так - `http://127.0.0.1:8888/lab?token=a5a753435ee9c5b564a54478e900602c67c2e5a41defe725`, то есть сервер разворачивается на локалхосте)
3. Последовательно выполнить все ячейки сначала в ноутбуке `DataToSnowflake.ipynb`, а потом в `ClickHouseReports.ipynb` (оба ноутбука лежат в папке work)
